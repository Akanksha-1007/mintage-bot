...
export default function BotDesignEditor({
  ...
  const handleLauncherLogoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      window.alert('Please select an image file for the launcher logo.');
      event.target.value = '';
      return;
    }

    if (file.size > 2 * 1024 * 1024) {
      window.alert('Launcher logo must be 2 MB or smaller.');
      event.target.value = '';
      return;
    }
    ...
    reader.onload = () => {
      ...
      if (value) updateConfig({ launcherLogoUrl: value });
    };
    reader.readAsDataURL(file);
    event.target.value = '';
  };

  const clearLauncherLogo = () => updateConfig({ launcherLogoUrl: '' });
          ...
          {activeSection === 'launcher' && (
            <div className="space-y-6">
              ...
              <div className="space-y-2 pt-1">
                <label className="block text-xs font-semibold text-slate-700">Launcher Logo</label>
                <div className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl bg-slate-50">
                  <div
                    className="w-12 h-12 rounded-xl flex items-center justify-center overflow-hidden border border-slate-200 shrink-0"
                    style={{ background: config.accentColor || '#5B3DF5' }}
                  >
                    {config.launcherLogoUrl ? (
                      <img src={config.launcherLogoUrl} alt="Launcher logo" className="w-full h-full object-contain bg-white p-1.5" />
                    ) : (
                      <span className="text-white">{renderLauncherIcon(config.launcherIcon || 'chat', 'w-5 h-5')}</span>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-semibold text-slate-700">Use your own logo inside the launcher</p>
                    <p className="text-[10px] text-slate-500 mt-0.5">PNG, JPG, WEBP or SVG • max 2 MB</p>
                    <div className="flex items-center gap-2 mt-2">
                      <label className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 cursor-pointer">
                        <Upload className="w-3.5 h-3.5" />
                        {config.launcherLogoUrl ? 'Change logo' : 'Add logo'}
                        <input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={handleLauncherLogoUpload} className="hidden" />
                      </label>
                      {config.launcherLogoUrl && (
                        <button type="button" onClick={clearLauncherLogo} className="text-[11px] font-semibold text-red-600 hover:text-red-700">Remove</button>
                      )}
                    </div>
                  </div>
                </div>
                ...
                >
                  {config.launcherLogoUrl ? (
                    <img
                      ...
                      alt="Launcher logo"
                      className="w-8 h-8 object-contain rounded-md bg-white/95 p-1"
                    />
                  ) : (
